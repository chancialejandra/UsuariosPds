...
public class TareaService implements ITareaService {
  ...
  public MessageResponse crearTarea(TareaRequest tareaRequest) {
    ...
    if (!nombreTareaExiste(tarea.getNombreTarea())) {
      ...
      responseMessage =
          MessageResponse.builder().message("Tarea guardada").status(HttpStatus.OK).build();
    } else {
      responseMessage =
          MessageResponse.builder()
              .message(
                  "No se puede guardar la tarea, ya existe"
                      + " una tarea con el nombre "
  ...
  public MessageResponse eliminarTarea(Long id) {
    ...
    if (optionalTarea.isPresent()) {
      ...
      responseMessage =
          MessageResponse.builder().message("Tarea elimidada").status(HttpStatus.OK).build();
    } else {
      responseMessage =
          MessageResponse.builder()
  ...
  public MessageResponse editarTarea(TareaRequest tareaRequest, Long id) {
    ...
    if (!optionalTarea.isPresent()) { // La no tarea existe
      ...
      responseMessage =
          MessageResponse.builder()
              .message(
                  "No se puede editar, ya existe tarea con "
                      + "un nombre: "
                      + tareaRequest.getNombre())
              .status(HttpStatus.BAD_REQUEST)
              .build();
    } else { // Edita la tarea
      ...
      responseMessage =
          MessageResponse.builder().message("Tarea editada").status(HttpStatus.OK).build();
    }
    return responseMessage;
  }
