...
public class UsuarioService implements IUsuarioService {
  ...
  public int[] mayorDeEdad(Date fechaNacimiento) {
    ...
    try {
      if ((anoSyst < anoUser)
          || (anoSyst == anoUser && mesSyst > monthUser)
    ...
    try {
      if (!findByNumeroIdenficacion(usuario.getNumeroIdentificacion())) {
        ...
        if (aniosUser[0] == 0 && aniosUser[1] == 0 && aniosUser[2] == 0) {
          ...
          responseMessage =
              MessageResponse.builder().message("Registro exitoso").status(HttpStatus.OK).build();
  ...
  public MessageResponse eliminarUsuario(String identificacion) {
    ...
    Optional<Usuario> optionalUsuario =
        iUsuarioRepository.findByNumeroIdentificacion(identificacion);

    if (optionalUsuario.isPresent()) {
      iUsuarioRepository.deleteById(optionalUsuario.get().getIdUsuario());
      responseMessage =
          MessageResponse.builder().message("Usuario elimidado").status(HttpStatus.OK).build();
    } else {
      responseMessage =
          MessageResponse.builder()
              .message(
                  "No se puede eliminar el usuario, no existe usuario con"
                      + " un Numero de indentifiacación: "
                      + identificacion)
              .status(HttpStatus.BAD_REQUEST)
              .build();
    }
  ...
  public UsuarioResponse actualizarUsuario(
      UsuarioRequest usuarioRequest, String numeroIdentificacion) {
    ...
    try {
      if (userValidation.isPresent()) {
        ...
        oldUser.setActivo(usuarioRequest.isCondicion());
